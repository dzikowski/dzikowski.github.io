...
Zarówno nadawca, jak i dobiorca powiązany jest z jednym tematem.
...
Odbiorcy muszą jedynie znać swoją pozycję w rejestrze -- indeks ostatnio odczytanego komunikatu.

Pojedyncza partycja musi w całości zmieścić się na jednym serwerze, musi być możliwe obsłużenie jej przez jeden broker.
...
Twórcy gwarantują wydajne działanie Kafki nawet dla 10&nbsp;000 partycji -- dla takiej ilości przeprowadzają testy wydajnościowe.
...
W tym przykładzie dla każdego tematu tworzona jest jedna partycja, stąd też mamy partycje A i B, odpowiadające tematom A i B.
...
Broker B2 jest liderem dla tematu A, to znaczy, że jest w rzeczywistości to on jest odpowiedzialny za wszystkie operacje zapisu i odczytu do partycji A.
...
Ważny jest też folder <code class="language-java" data-lang="java">{% highlight java nowrap %}conf{% endhighlight %}</code>, w którym znajdują się odpowiednie pliki konfiguracyjne dla instancji Zookeepera oraz Kafki.
...
Przygotowałem takie pliki osobno dla [Linuksa](https://github.com/dzikowski/simple-kafka-storm-java/tree/master/conf-linux) i [Windowsa](https://github.com/dzikowski/simple-kafka-storm-java/tree/master/conf-windows).
...
Aby uruchomić Kafkę dla celów testowych, najlepiej skopiować przygotowane przeze mnie pliki konfiguracyjne Kafki i Zookeepera do folderu <code class="language-java" data-lang="java">{% highlight java nowrap %}conf{% endhighlight %}</code>, znajdującego się w dystrybucji Kafki, a następnie uruchomić przygotowany przeze mnie skrypt.
...
W pierwszej kolejności uruchamiam instancję Zookeepera, którego na szczęście nie muszę instalować -- dystrybucja Kafki ma wersję Zookeepera, która w zupełności nam wystarczy. 
Następnie uruchamiam pierwszą instancję Kafki i czekam przez chwilę, żeby zdążył się uruchomić.
...
Przykład nadawcy w Javie znajdziesz w klasie 
  [<code class="language-java" data-lang="java">{% highlight java nowrap %}RandomTransactionProducer{% endhighlight %}</code>](https://github.com/dzikowski/simple-kafka-storm-java/blob/master/src/main/java/io/github/dzikowski/kafka/RandomTransactionProducer.java), a przykład jego uruchomienia w [<code class="language-java" data-lang="java">{% highlight java nowrap %}KafkaProduceExample{% endhighlight %}</code>](https://github.com/dzikowski/simple-kafka-storm-java/blob/master/src/main/java/io/github/dzikowski/apps/KafkaProduceExample.java).
...
Stworzyłem metodę <code class="language-java" data-lang="java">{% highlight java nowrap %}start{% endhighlight %}</code>, w której tworzę obiekt nadawcy, podając jako argumenty liczbę transakcji, które mają być wygenerowane, temat, do którego mają być przesyłane komunikaty (większość stałych trzymam w oddzielnym interfejsie, [<code class="language-java" data-lang="java">{% highlight java nowrap %}Conf{% endhighlight %}</code>](https://github.com/dzikowski/simple-kafka-storm-java/blob/master/src/main/java/io/github/dzikowski/apps/Conf.java)), a także obiekt konfiguracji nadawcy.
Ponieważ nadawca implementuje interfejs <code class="language-java" data-lang="java">{% highlight java nowrap %}Runnable{% endhighlight %}</code>, mogę go uruchomić w nowym wątku.

Konfiguracja nadawcy zawarta jest w obiekcie klasy <code class="language-java" data-lang="java">{% highlight java nowrap %}Properties{% endhighlight %}</code>, dla porządu tworzę ten obiekt w osobnej metodzie.
...
Kolejno podaję parametry:

  - Listę broketów Kafki.
  ...
  - Klasę, której obiekty mają służyć do ustalania, do której z partycji ma trafić dany komunikat. [<code class="language-java" data-lang="java">{% highlight java nowrap %}VerbosePartitioner{% endhighlight %}</code>](https://github.com/dzikowski/simple-kafka-storm-java/blob/master/src/main/java/io/github/dzikowski/kafka/VerbosePartitioner.java) to napisana przeze mnie prosta klasa, która losowo rozdziela komunikaty do dostępnych partycji, a po drodze wypisuje informację o tym na standardowym wyjściu. Dzieki temu możesz się przekonać, że sam mechanizm rozdzielania działa, a także ile partycji faktycznie zostało stworzonych.
...
Zostaną one wykorzystane w metodzie <code class="language-java" data-lang="java">{% highlight java nowrap %}run{% endhighlight %}</code> nadawcy.
...
W metodzie <code class="language-java" data-lang="java">{% highlight java nowrap %}run{% endhighlight %}</code> najpierw tworzony jest obiekt nadawcy Kafki, następnie wysyłam za pośrednictwem tego obiektu daną liczbę losowych transakcji.
Żeby sam proces trwał dłużej -- a tym samym, żeby można lepiej śledzić jego działanie -- co 100 wygenerowanych transakcji, nadawca czeka 300&nbsp;milisekund.

Komunikaty wysyłane są jako obiekty etodzie <code class="language-java" data-lang="java">{% highlight java nowrap %}KeyedMessage{% endhighlight %}</code>.
Obiekty takie mają przypisany temat, klucz komunikatu i samą treść komunikatu.
Klucz może służyć między innymi do rozdzielania komunikatów na poszczególne partycje.
Gdyby nie był podany, <code class="language-java" data-lang="java">{% highlight java nowrap %}VerbosePartitioner{% endhighlight %}</code> w ogóle nie byłby wywoływany.

Aby przykład nadawcy zadziałał, najpierw musisz mieć uruchomioną Kafkę za pośrednictwem przygotowanego skryptu.
Potem uruchom klasę <code class="language-java" data-lang="java">{% highlight java nowrap %}KafkaProduceExample{% endhighlight %}</code>, a na standardowym wyjściu programu zobaczysz transakcje (w formacie JSON) oraz informacje o ich przypisaniu do określonych partycji.
...
Wczesniej wspomniałem o sposobie dostarczania komunikatów _publish-subscribe_.
W tym podejściu Kafka emituje sekwencję komunikatów, które mogą odbierać dowolni odbiorcy.
Wszyscy odbiorcy z danego tematu odbierają wszystkie komunikaty z tego tematu.
...
Kafka zapewnia taki sposób dostarczania komunikatów w ramach grup odbiorców.
...
Przykład uruchomienia grupy odbiorców znajdziesz w klasie [<code class="language-java" data-lang="java">{% highlight java nowrap %}KafkaProduceExample{% endhighlight %}</code>](https://github.com/dzikowski/simple-kafka-storm-java/blob/master/src/main/java/io/github/dzikowski/apps/KafkaConsumerGroupExample.java).
...
Na początku tworze obiekt stworzonej przeze mnie klasy reprezentującej grupę odbiorców, podając temat oraz obiekt konfiguracji.
Następnie uruchamiam grupę trzech odbiorców, która nasłuchuje przez 20 sekund, a potem kończę jej działanie.

Spójrz jeszcze na konfigurację grupy odbiorców, a potem na samą klasę [<code class="language-java" data-lang="java">{% highlight java nowrap %}ConsumerGroup{% endhighlight %}</code>](https://github.com/dzikowski/simple-kafka-storm-java/blob/master/src/main/java/io/github/dzikowski/kafka/ConsumerGroup.java).
...
Aby przetestować cały proces, możesz uruchomić aplikację <code class="language-java" data-lang="java">{% highlight java nowrap %}KafkaProduceAndConsumeExample{% endhighlight %}</code> z pakietu <code class="language-java" data-lang="java">{% highlight java nowrap %}io.github.dzikowski.apps{% endhighlight %}</code>.
Aplikacja uruchamia najpierw KafkaProduceExample dla 3000 losowych transakcji, a następnie KafkaConsumerGroupExample.

W konsekwencji w oknie terminala -- odbiorcy tematu coolkafka-in pojawią się komunikaty w postaci transakcji w formacie JSON.
Takie same komunikaty pojawią się też na wyjściu aplikacji, wraz z informacją o przydzieleniu do konkretnego wątku (czyli informacja z klasy VerbosePartitioner).
...
---
  ...
  - **Nadawca** (ang. _consumer_) W kontekście Kafki chodziło o coś, co służy do "produkowania" komunikatów. "Nadawca" wydaje mi się dużo lepszym określeniem, niż "producent".
